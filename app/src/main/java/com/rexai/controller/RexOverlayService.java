...
public class RexOverlayService extends Service {
    ...
    private void processSmartCommand(String text) {
        ...
        if (cmd.contains("light on") || cmd.contains("torch on") || cmd.contains("लाईट चालू") || cmd.contains("टॉर्च ऑन")) {
            toggleFlashlight(true);
            showToast("REX: Torch ON");
            return;
        }
        if (cmd.contains("light off") || cmd.contains("torch off") || cmd.contains("लाईट बंद") || cmd.contains("टॉर्च ऑफ")) {
        ...
        if (cmd.contains("camera") || cmd.contains("कैमरा")) {
            try {
        ...
        if (cmd.contains("settings") || cmd.contains("सेटिंग")) {
            try {
        ...
        if (cmd.contains("dialer") || cmd.contains("phone") || cmd.contains("फोन")) {
            try {
        ...
        String query = cmd.replace("rex", "").replace("youtube", "").replace("search", "").replace("play", "").replace("khol", "").replace("कर", "").replace("खोल", "").replace("वर", "").trim();
